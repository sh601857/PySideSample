...
class MyMplCanvas(FigureCanvas):
    ...
    def __init__(self, parent=None, width=9, height=6, dpi=None):
        fig = Figure(figsize=(width, height), facecolor=(.94,.94,.94), dpi=dpi)
        FigureCanvas.__init__(self, fig)
        self.setParent(parent)
        ...
        axcolor = 'lightgoldenrodyellow'
        self.axes = fig.add_axes([0.06, 0.10, 0.9, 0.85], axis_bgcolor=(.94,.94,.94))
        
        self.compute_initial_figure()
        
        axxspan = fig.add_axes([0.06, 0.05, 0.9, 0.02])
        ...
        axyspan = fig.add_axes([0.02, 0.10, 0.01, 0.85])
        ...
        axReset = fig.add_axes([0.01, 0.05, 0.03, 0.03],frameon=False, )
